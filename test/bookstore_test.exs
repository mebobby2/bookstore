defmodule BookstoreTest do
  ...
  def friendly_unicode do
    bad_chars = [<<0>>, "\\", "_", "%"]
    friendly_gen =
      such_that s <- non_empty(utf8(256)), when: (not contains_any?(s, bad_chars)) && String.length(s) < 256
  ...
  def postcondition(state, {_, _, :find_book_by_isbn_exists, [isbn]}, {:ok, [res]}) do
